...
int* Benchmark::computeSP(const string ShortLeads) { //각 shortRead의 한 문장씩..?

	int k = ShortLeads.size();  //패턴의 길이 

	int* sp = new int[k];	//prefix와 suffix 테이블 만들기

	for (int j = 1; j <= k - 1; j++) {
		while (k > 0 && ShortLeads.at(k) != ShortLeads.at(j)) // k=0부터 시작해서,
			k = sp[k - 1];  // k=0 부터 시작해서
		if (ShortLeads.at(k) == ShortLeads.at(j))
			sp[j] = ++k;
